...
async function main() {
    ...
    console.log(nff1.address);
    console.log(nff2.address);
    ...
    //let encodedOrder = [encodedErc20Order, encodedErc721Order];

    const orderHash = await verifySignature.getMessageHash(owner, actions, encodedOrder, nonce, expiration);

    const signedOrderMsg = await web3.eth.sign(orderHash, owner, console.log);

    await metaExchange.connect(addr1_e).fill(maker, taker, actions, encodedOrder, signedOrderMsg, nonce, expiration);
}
