...
describe("Do a transaction of basket of assets of different types", function() {
    ...
    it("Should succeed if transaction is made by taker", async() => {
        ...
        let order = [
            makerErc20Addresses,
            makerErc20Amounts,
            makerErc721Addresses,
            makerErc721Ids,
            makerErc1155Addresses,
            makerErc1155Ids,
            makerErc1155Amounts,
            takerErc20Addresses,
            takerErc20Amounts,
            takerErc721Addresses,
            takerErc721Ids,
            takerErc1155Addresses,
            takerErc1155Ids,
            takerErc1155Amounts,
            expiration
        ];
        let encodedOrder = web3.eth.abi.encodeParameters(orderTypes, order);
        const orderArgs = [owner, encodedOrder, expiration, nonce];

        const orderHash = await verifySignature.getMessageHash(...orderArgs);
        const signedOrderMsg = await web3.eth.sign(orderHash, owner, null);

        await expect(metaExchange.connect(addr1_e).
            fill(makerAddress, takerAddress, encodedOrder, signedOrderMsg, nonce)).to.not.be.reverted;
    })
